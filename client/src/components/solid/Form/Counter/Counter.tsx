...
export function Counter(props: ICounterProps) {
  ...
  function changeHandler(e) {
    if (parseInt(e.currentTarget.value, 10) < 0) setValue(0);
  ...
  return (
    <div class={styles.counter}>
      ...
      <div class={styles.counter__counter}>
        <button class={styles.counter__btn} type="button" onClick={decr} disabled={!value()}>
          <svg width="12" height="3" viewBox="0 0 12 3" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M1 1.5H11" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
          </svg>
        </button>
        <input size={2} class={styles.counter__input} type="number" name={props.label} value={value()} onChange={(e) => changeHandler(e)} />
        <button class={styles.counter__btn} type="button" onClick={incr}>
